...
data. The preferred *method* here is "MB_Timeseries", which stands for 'multi band 
time series'. Use this method, if you provide multiple single band rasters or raster 
stacks with different time stamps per scene. The functions sorts them by date and 
...
classifier = Vector of all model variants to be used. In this case random 
forest = "rf", partial-least-squares = "pls", neural networks = "nnet" and 
...
Once all parameters are set, one can run the modelworkflow `calc.model` like this:
