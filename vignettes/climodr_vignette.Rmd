...
# Next, the stations have to be located in space, with a coordinate system. This step is crucial to process the data in a modelation usecase.\
...
aoalist <- list.files(file.path(envrmt$path_predictions, "aoa"), pattern = ".tif")

head(predlist)
```
Now lets plot the second one, which was the linear regression model for temperature. 
```{r plot predictions}
pred <- terra::rast(file.path(envrmt$path_predictions, predlist[2]))
terra::plot(pred, main = "LLO_lim_tem")
```
Now lets apply the aoa on the prediction, to see, which areas are applicable.
```{r}
aoa <- terra::rast(file.path(envrmt$path_predictions, "aoa", aoalist[2]))
terra::plot(pred, main = "LLO_lim_tem + AOA")
terra::plot(aoa, add = TRUE)
```
Note: It works, the aoa says that the whole prediction is not applicable. 
